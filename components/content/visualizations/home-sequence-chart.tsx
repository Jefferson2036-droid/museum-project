...
const supportLayers = [
  {
    ...
    description: "Papers and source documents explain why each era hands its methods forward.",
  },
  {
    label: "Cast",
    href: "/people-and-institutions",
    description: "Researchers, labs, and public institutions make the sequence inhabited instead of abstract.",
  },
  {
    label: "Concepts",
    href: "/guides/embeddings-latent-space-and-llm-math",
    description: "Guides such as embeddings and latent space slow the modern story down at the right point.",
  },
];
...
export function HomeSequenceChart({ compact = false }: HomeSequenceChartProps) {
  return (
    <figure
      className={cn("home-sequence-chart", compact && "home-sequence-chart--compact")}
    ...
    >
      <div className="home-sequence-chart__header">
        ...
        <h3 id="home-sequence-chart-title">The site teaches one long handoff, not disconnected islands</h3>
        <p>
          The chronology is the spine. The paper map, cast page, and concept guides keep changing the scale
          without breaking that sequence.
        </p>
      </div>
      ...
      {compact ? null : (
        <div className="home-sequence-chart__layers" aria-label="Companion layers around the chronology">
          {supportLayers.map((layer) => (
            <Link key={layer.label} href={layer.href} className="home-sequence-chart__layer">
