...
function HeroVisualLink({ href, eyebrow, title, imageUrl, imageAlt, variant }: HeroVisual) {
  ...
  return (
    ...
    >
      ...
      <div className={cn("absolute inset-x-0 bottom-0 bg-[image:var(--gradient-image-overlay)] text-white", isFeature ? "p-5 sm:p-6" : "p-4 sm:p-5")}>
        ...
        <HomePanelSplit
          ...
          left={
            <div className="space-y-6 sm:space-y-7" style={openingTypeScale}>
              <HomeEyebrow scale="section" style={{ fontSize: "var(--opening-step-negative-1)" }}>
                {eyebrow}
              </HomeEyebrow>
              <h1 aria-label={titleLines.join(" ")} className="font-(family-name:--font-display) tracking-[-0.045em] text-foreground" style={{ fontSize: "clamp(3.2rem, 2.4rem + 2.4vw, var(--opening-step-4))", lineHeight: "0.89", maxWidth: "9.4ch" }}>
                {titleLines.map((line) => (
              ...
              <p className="text-(--ink-84)" style={{ fontSize: "clamp(1.04rem, 0.98rem + 0.35vw, var(--opening-step-1))", lineHeight: "var(--opening-body-leading)", maxWidth: "100%", width: "100%" }}>
                {deck}
              </p>
