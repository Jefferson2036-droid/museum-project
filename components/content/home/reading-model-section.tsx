...
import { HomeEyebrow, homePillStyles, homeTokens } from "@/components/content/home/home-design-system";
...
function CompanionRouteLink({ href, eyebrow, title, description, icon: Icon }: CompanionRoute) {
  return (
    <Link href={href} className="@container/reading-card group flex h-full flex-col justify-between bg-white/3 p-5 transition hover:bg-white/6 sm:p-6">
      <div className="space-y-3">
        <div className="flex items-start justify-between gap-4">
          <HomeEyebrow tone="inverse">{eyebrow}</HomeEyebrow>
          <Icon className="mt-0.5 size-4 shrink-0 text-white/54" />
        </div>
        <h3 className="max-w-[15ch] font-(family-name:--font-display) text-[1.32rem] leading-[0.96] tracking-[-0.032em] text-white transition group-hover:text-white/88 sm:text-[1.42rem]">{title}</h3>
      </div>
...
export function ReadingModelSection({
  ...
  return (
    <Panel id="home-reading-model" className="overflow-hidden border-(--ink-18) bg-[image:var(--gradient-dark-reading)] text-white shadow-[0_20px_68px_rgba(27,18,15,0.16)]">
      <HomePanelSplit
        ...
        left={
          <>
            <div className="space-y-5">
              ...
              <h2 className="max-w-[10ch] font-(family-name:--font-display) text-[2.45rem] leading-[0.93] tracking-[-0.04em] text-white sm:text-[2.8rem] xl:text-[3.02rem]">{heading.title}</h2>
          ...
          <>
            <div className="grid gap-4 border-b border-white/12 p-6 sm:p-7 xl:grid-cols-[minmax(17rem,0.58fr)_minmax(0,1fr)] xl:items-center xl:p-8">
              <div className="space-y-2">
                ...
                <h3 className="max-w-[14ch] font-(family-name:--font-display) text-[1.55rem] leading-[0.97] tracking-[-0.03em] text-white sm:text-[1.7rem]">{promptBlock.title}</h3>
              </div>
